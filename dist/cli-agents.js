...
function isCodexUnsupportedChatGPTModelError(error, requestedModel) {
    if (!requestedModel)
        return false;
    const combined = `${error.message || ''}\n${error.stdout || ''}\n${error.stderr || ''}`;
    return /model['"\\\s:A-Za-z0-9._-]*not supported when using Codex with a ChatGPT account/i.test(combined);
}
function isCodexUnsupportedModelResponse(response, requestedModel) {
...
export class CLIAgentOrchestrator {
    ...
    defaultTimeout = DEFAULT_TIMEOUT; // honors BRUTALIST_TIMEOUT env
    ...
    async executeSingleCLI(cli, userPrompt, systemPromptSpec, options = {}) {
        ...
        try {
            ...
            if (cli === 'codex' && isCodexUnsupportedModelResponse(response, requestedCodexModel)) {
                ...
                const fallback = await this._executeCLI(cli, userPrompt, systemPromptSpec, fallbackOptions, (user, sys, opts) => this.buildCLICommand(cli, user, sys, opts));
                if (fallback.success) {

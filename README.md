...
Each entry's token is placed in a dedicated env var and referenced via `authTokenEnv` — raw tokens are never inlined into the forwarded `BRUTALIST_CLAUDE_CLIENTS`. Every client gets an isolated `~/.brutalist/claude-clients/<id>` config dir (mode `0700`), is isolated from native credentials, and suppresses caller-requested MCP by default exactly like the `roast` `clients[]` above. The legacy `"hardened"` label does **not** mean egress-sandboxed: Bash, `WebFetch`, and `WebSearch` remain available. Each **native** critic reviews the whole diff chunked to its **own** context window (claude ~1M with `[1m]`, codex 272k, agy ~135k), every stream additionally capped by the orchestrator brain's window (1M with `[1m]` on `model`, else ~200k, since the brain reads every chunk). The routed custom clients run as **one** stream sized to the **smallest** of their declared `contextWindow`s — so a *single* custom client effectively gets its own window, but *multiple* customs share one stream (sized to the smallest, to avoid overflowing any). A small window no longer forces finer chunks on the other critics — so set each client's `contextWindow` to its model's usable window. When the whole diff already fits the smallest active window, all critics collapse into a single shared pass.

...
export class ToolHandler {
    ...
    async handleRoastTool(config, args, extra) {
        try {
            ...
            const pageReadRequested = args.context_id !== undefined &&
            ...
            if (args.context_id && !args.force_refresh) {
                ...
                if (cachedResponse) {
                    ...
                    if (args.resume === true && !pageReadRequested) {
                        ...
                        if (args.resume === true) {
                            logger.warn('Ignoring resume=true on page-read request; context_id + offset/cursor returns cached content');
                        }
            ...
            const validatedPrimaryArg = primaryArg;
            ...
            // Run the analysis
            const result = await this.executeBrutalistAnalysis(config.analysisType, validatedPrimaryArg, systemPrompt, context, args.workingDirectory, args.clis, args.verbose, args.models, progressToken, sessionId, requestId, args.mcp_servers);
     ...
     */
    async executeBrutalistAnalysis(analysisType, primaryContent, systemPromptSpec, context, workingDirectory, clis, verbose, models, progressToken, sessionId, requestId, mcpServers) {

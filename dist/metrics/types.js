 ...
 */
export function serializeLabels(labelNames, values) {
    ...
    for (const key of labelNames) {
        ...
        if (raw === undefined) {
            throw new Error(`metrics: missing label "${key}" for metric (expected keys: ${labelNames.join(', ')})`);
        }
 ...
 * codebase build label values from closed sets (`claude`/`codex`/`agy`,

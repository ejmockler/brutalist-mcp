...
import { GeminiServerConfig, GeminiModel, GeminiPromptOptions, GeminiResponse } from "./types/index.js";

const execAsync = promisify(exec);

export class GeminiServer {
  public server: McpServer;
  public config: GeminiServerConfig;

  private readonly AVAILABLE_MODELS: GeminiModel[] = [
  ...
  private registerTools() {
    ...
    this.server.tool(
      "gemini_prompt",
      "Execute a prompt using the Gemini CLI with specified model and options",
      {
    ...
    this.server.tool(
      "gemini_models",
      "List available Gemini models and their information",
      {},
      async () => {
        const modelsList = this.AVAILABLE_MODELS.map(model => 
          `${model.name}: ${model.description} (${model.available ? 'Available' : 'Unavailable'})`
        ).join('\n');
    ...
    this.server.tool(
      "gemini_with_stdin",
      "Process input data through Gemini with a prompt",
      {
    ...
    this.server.tool(
      "gemini_json",
      "Execute prompt and get structured JSON response",
      {
        prompt: z.string().describe("The prompt to send to Gemini"),
        model: z.string().optional().describe("Gemini model to use"),
    ...
    this.server.tool(
      "gemini_directories",
      "Run Gemini with access to multiple directories",
      {
  ...
  private async executeGeminiPrompt(options: GeminiPromptOptions): Promise<GeminiResponse> {
